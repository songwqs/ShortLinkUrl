...
try {
    ...
    //追加机场订阅转换api
    route('/short', function () {
    $longUrl = $_REQUEST['longUrl'] ?? '';
    $decodedUrl = base64_decode($longUrl);

        if (empty($decodedUrl)) {
            // URL 为空或无效时的错误响应
            header('Content-Type: application/json');
            http_response_code(400);
            echo json_encode(['Code' => 0, 'Message' => 'Invalid or empty URL']);
            return;
        }
    
        $su = new \Libs\ShortURL();
        $result = $su->urlToShort($decodedUrl, ['normal'], []);
    
        if (is_string($result)) {
            // 成功生成短链接
            header('Content-Type: application/json');
            http_response_code(200);
            echo json_encode(['Code' => 1, 'ShortUrl' => $result]);
        } else {
            // 返回未知错误
            header('Content-Type: application/json');
            http_response_code(400);
            echo json_encode(['Code' => 0, 'Message' => 'Unknown error']);
        }
    });
